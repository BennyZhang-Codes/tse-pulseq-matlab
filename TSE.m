...
fov = 256e-3;
nX=128; nY=128; nEcho=16; nSlice=5;
rflip = 180;
...
for iexcit = 0:nExcit % MZ: we start at 0 to have one dummy
    ...
    for isli = 1:nSlice
        rfex.freqOffset   = GSex.amplitude  * sliceThickness * (isli-1-(nSlice-1)/2);
        rfref.freqOffset  = GSref.amplitude * sliceThickness * (isli-1-(nSlice-1)/2);
